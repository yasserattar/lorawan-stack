...
func (x *Gateway) MarshalProtoJSON(s *jsonplugin.MarshalState) {
	...
	if x.DisablePacketBrokerForwarding {
		s.WriteMoreIf(&wroteField)
	...
	s.ReadObject(func(key string) {
		...
		case "gateways":
			s.ReadArray(func() {
				if !s.ReadNil() {
					v := &Gateway{}
					v.UnmarshalProtoJSON(s.WithField("gateways"))
	...
	s.ReadObject(func(key string) {
		...
		case "antenna_locations", "antennaLocations":
			s.ReadArray(func() {
				if !s.ReadNil() {
					v := &Location{}
					v.UnmarshalProtoJSON(s.WithField("antenna_locations"))
	...
	s.ReadObject(func(key string) {
		...
		case "last_status", "lastStatus":
			if !s.ReadNil() {
				...
				x.LastStatus.UnmarshalProtoJSON(s.WithField("last_status"))
			}
