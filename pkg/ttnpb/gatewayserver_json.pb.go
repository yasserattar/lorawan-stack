	...
	s.ReadObject(func(key string) {
		...
		case "uplink_messages", "uplinkMessages":
			s.ReadArray(func() {
				if !s.ReadNil() {
					v := &UplinkMessage{}
					v.UnmarshalProtoJSON(s.WithField("uplink_messages"))
		...
		case "gateway_status", "gatewayStatus":
			if !s.ReadNil() {
				...
				x.GatewayStatus.UnmarshalProtoJSON(s.WithField("gateway_status"))
			}
		case "tx_acknowledgment", "txAcknowledgment":
			if !s.ReadNil() {
				x.TxAcknowledgment = &TxAcknowledgment{}
				x.TxAcknowledgment.UnmarshalProtoJSON(s.WithField("tx_acknowledgment"))
			}
		}
	})
}
